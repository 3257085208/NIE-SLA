...
本目录是 NIE-SLA 的生产前端源，包含公开状态页、管理后台、Agent 安装器、External Latency Agent 和已校验的发布二进制。

新部署由 `agent/worker/scripts/prepare-assets.mjs` 复制为 Worker Static Assets，与 API 同源发布。目录中的 Pages Functions 只为旧 Pages + Worker 部署迁移期兼容保留，不是新用户的部署主线。
...
生产发布使用 `deploy.sh` 完成同样步骤。生成的 `dist-one-click` 不得包含 `AGENTS.md`、tests、functions、node_modules、开发锁文件或已删除的主题文件。
...
公开页展示 Cloudflare 状态、Agent 在线与指标、多来源 Latency、Ping、流量、账单、自动 GeoIP 位置和 IPv4 解锁摘要。

后台提供目标管理、商家与自定义商家、机器类型、GeoIP 服务商、固定 Beta 动作、Telegram/邮件、外观、更新、备份与恢复。

主题/插件上传、包运行时和旧卡片主题已移除。内置前端仍支持站点名、Favicon、Logo、页头内容、颜色、文案与区域显隐。
...
还应在桌面与窄屏移动端检查公开页、NQ 旧报告弹窗、IP 解锁摘要、VPS 详情折叠、图表缩放和后台表单。
