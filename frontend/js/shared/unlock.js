...
export function unlockState(service) {
  ...
  if (/失败|未解锁|不支持|封锁|屏蔽|禁止|禁会员|中国/.test(status) || /(?:^|\s)(?:blocked|failed|no|unsupported|restricted)(?:\s|$)/.test(status)) return 'bad';
  if (/仅\s*自制|仅\s*网页|仅\s*app|部分|partial/.test(status)) return 'dns';
