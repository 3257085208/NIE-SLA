...
assert.match(adminHtml, /href="\/admin\.css\?v=20260731-v1046"/, 'custom admin paths must load CSS from the site root');
...
assert.match(adminHtml, /src="\/js\/admin-bootstrap\.js\?v=20260731-v1046"/, 'admin login must install a startup failure guard');
assert.match(adminHtml, /src="\/js\/admin\.js\?v=20260731-v1046"/, 'custom admin paths must load the admin module from the site root');
...
assert.match(adminSource, /JSON\.stringify\(\{ agent_ids: ids, action \}\)/, 'batch Agent tasks must submit the selected VPS IDs in one request');
...
assert.match(adminSource, /install-command\.js\?v=20260731-v1046/, 'Agent install clipboard fixes must use the current cache key');
...
assert.match(indexHtml, /app\.js\?v=20260731-v1046/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260731-v1046/, 'frontend CSS cache key must publish the current release');
