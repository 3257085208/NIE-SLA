...
assert.match(appSource, /if \(\/dns\/\.test\(method\)\) return 'dns'/, 'DNS unlocks must have a distinct visual state');
assert.match(appSource, /<div class="service-uptime">\$\{uptime\}<\/div>\s*\$\{unlockStrip\}\s*<div class="service-bottom-row/, 'unlock checks must sit between runtime status and the uptime bars');
...
assert.match(styleSource, /\.unlock-strip\s*\{[\s\S]*grid-template-columns:\s*repeat\(6/, 'unlock checks must stay in one six-column row');
assert.match(styleSource, /\.unlock-pill\.dns\s*\{[\s\S]*#fff8df/, 'DNS unlock checks must use the yellow state');
