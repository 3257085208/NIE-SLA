...
assert.match(adminSource, /import \{ nqOptionsHtml, readNqOptions \} from "\.\/admin\/nq-options\.js\?v=20260925-visual1"/, 'admin must import the configurable NodeQuality option helpers');
assert.match(adminSource, /import \{ createOnboarding \} from "\.\/admin\/onboarding\.js\?v=20260925-visual1"/, 'admin must import the first-run onboarding module');
...
assert.match(indexHtml, /id="checksToggle"/, 'the recent-checks panel must expose a collapse toggle');
assert.match(appSource, /function setChecksCollapsed/, 'the collapse state must be handled and persisted in app.js');
assert.match(appSource, /function latencyLevelClass/, 'slow latency values must get a visible level class');
assert.match(styleSource, /\.checks-panel\.collapsed #checks/, 'the collapsed panel must hide its records');
assert.match(styleSource, /\.daybar\.full \{ background: #cfe8da/, 'normal day bars must use the calmed palette');
...
assert.match(adminSource, /agentRootlessInstallCommandFromPayload\(d, t\.id\)/, 'deploy must offer the rootless install variant alongside the primary command');
assert.match(adminSource, /install-mode-modal/, 'agent deploy must present root and rootless install modes');
assert.match(adminSource, /data-copy-cmd/, 'install modes must expose one-click copy buttons');
...
assert.match(appSource, /normalizeBackrouteEntries\(t\.backroute\)/, 'public cards must normalize stored backroute records');
assert.match(appSource, /backroute-summary/, 'public cards must place backroute badges beside NQ');
assert.doesNotMatch(appSource, /systemItems\.push\(vpsItem\('回程线路'/, 'backroute must not remain as a detached VPS detail row');
...
assert.match(adminHtml, /href="\/admin\.css\?v=20260925-visual1"/, 'custom admin paths must load CSS from the site root');
...
assert.match(adminHtml, /src="\/js\/admin-bootstrap\.js\?v=20260925-visual1"/, 'admin login must install a startup failure guard');
assert.match(adminHtml, /src="\/js\/admin\.js\?v=20260925-visual1"/, 'custom admin paths must load the current admin module from the site root');
...
assert.match(adminSource, /repair_needed === true/, 'admin must surface Agents that cannot self-update');
assert.match(adminSource, /id="disableTotpCode"/, 'disabling TOTP must require a verification code');
assert.doesNotMatch(adminSource, /confirm\("关闭 TOTP？"\)/, 'TOTP disable must not rely on a native confirm dialog');
...
assert.match(adminHtml, /id="retentionHours"[^>]*type="range"[^>]*min="72"[^>]*max="720"[^>]*step="24"/, 'Data retention must default to 72h with a 720h (30 day) ceiling');
assert.match(adminSource, /byId\("saveRetention"\)/, 'Retention saves must bind a dedicated admin button');
assert.match(adminSource, /"\/api\/retention-config"/, 'Admin must read and patch the retention endpoint');
assert.match(adminSource, /showRetentionStorageAdvice/, 'Raising retention must advise S3-compatible external storage');
...
assert.match(adminHtml, /settings-subnav[\s\S]*data-settings-tab="alerts"[\s\S]*data-settings-tab="system"/, 'settings must expose a secondary category menu');
assert.match(adminHtml, /data-settings-panel="alerts"[\s\S]*data-settings-panel="system"/, 'settings categories must own separate content panels');
assert.doesNotMatch(adminHtml, /settings-appearance|id="sAppearance"/, 'the original theme appearance editor must live in the theme center, not the system settings page');
...
assert.match(adminSource, /install-command\.js\?v=20260925-visual1/, 'Agent and Latency install clipboard fixes must use the current cache key');
...
assert.match(adminSource, /id="proxySaveStatus"/, 'proxy save failures must be visible inside the modal');
...
assert.match(adminSource, /function showAppUpdateGuide\(\)[\s\S]*NIE-SLA Online Update[\s\S]*Run workflow[\s\S]*部署仓库没有工作流[\s\S]*ONLINE_UPDATE_WORKFLOW_SNIPPET[\s\S]*长时间没有更新[\s\S]*openModal\(\)/, 'update guide must explain the no-workflow case, the paste-once snippet and the stuck-update checklist');
assert.match(adminSource, /ONLINE_UPDATE_WORKFLOW_SNIPPET = `name: NIE-SLA Online Update[\s\S]*uses: 3257085208\/NIE-SLA\/\.github\/workflows\/nie-sla-update\.yml@main/, 'the paste-once snippet must call the official reusable workflow');
assert.match(adminSource, /copyUpdateWorkflow[\s\S]{0,240}navigator\.clipboard\.writeText/, 'the update guide must offer a one-click copy for the workflow snippet');
assert.match(adminSource, /app-update-warn[\s\S]{0,320}GitHub Actions/, 'update center must state that the deployment repository Actions perform the upgrade');
...
assert.match(adminSource, /流量重置日与 VPS 到期时间完全独立|流量周期只按独立的重置日计算，与 VPS 到期时间互不影响/, 'traffic settings must explain expiry/reset separation');
...
assert.match(indexHtml, /app\.js\?v=20260925-visual1/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260925-visual1/, 'frontend CSS cache key must publish the current release');
...
assert.match(appSource, /else if \(metric === 'proc'\)[\s\S]*state\.chart\.data\.datasets = \[netDataset\('进程数'/, 'process chart must stay in the shared redraw path');
assert.match(appSource, /else if \(metric === 'temp'\)/, 'metric chart branches must keep process/load charts out of the temperature fallback chain');
assert.match(appSource, /state\.chart\.data\.datasets = \[netDataset\('进程数'[\s\S]*state\.chart\.options\.scales\.y\.beginAtZero/, 'process chart must apply the shared axis range and redraw after assigning data');
assert.match(appSource, /chartServiceName\.textContent = `\$\{state\.selectedName \|\| '服务'\} TCP Ping`/, 'TCP Ping chart cards must update their service title');
assert.match(appSource, /function chartAxisRange\(\)[\s\S]*selectedMetricRange === '24h'/, 'metric charts must format the 24-hour axis with date context');
assert.match(appSource, /metricsLoading[\s\S]*正在加载/, 'metric chart switches must expose a loading state instead of retaining stale data');
...
assert.match(adminSource, /openClassicThemeSettings[\s\S]*appearance-editor/, 'the original theme card must open the appearance editor from the theme center');
assert.match(adminSource, /function themeCardHtml\(theme\)[\s\S]*theme\.builtin/, 'theme cards must render builtin official themes like third-party cards');
assert.match(adminSource, /openThemeSettings\(theme\)/, 'third-party theme cards must keep their settings modal');
...
assert.match(appSource, /appearance-script\.js/, 'custom appearance scripts must load from the same-origin CSP-compatible endpoint');
assert.doesNotMatch(appSource, /script\.textContent = appearance\.custom_script/, 'custom appearance scripts must not be injected inline under script-src self');
assert.match(adminSource, /JSON\.stringify\(\{ appearance \}\)/, 'admin must save frontend appearance settings');
assert.match(adminHtml, /id="sProbeHistoryDeadLetters"/, 'admin must expose probe history dead-letter maintenance');
assert.match(adminSource, /dead-letter\/replay['"]/, 'admin must expose single dead-letter replay');
assert.match(adminSource, /dead-letter\/drain['"]/, 'admin must expose bounded dead-letter drain');
...
assert.match(themeSource, /THEME_CANVAS_LOAD_TIMEOUT_MS = 12_000/, 'slow canvas themes must have enough time to load through a cold Worker');
assert.match(themeSource, /await loaded;[\s\S]*canvas\.hidden = false[\s\S]*extensionThemeMode = 'canvas'/, 'canvas themes must stay hidden until their document is ready');
...
assert.match(appSource, /import \{ targetSlaPercentage \} from '\.\/js\/shared\/sla\.js\?v=20260925-visual1'/, 'VPS rows must calculate SLA with the tested shared helper');
...
assert.match(appSource, /function proxyDisplayDuration\([\s\S]*stage \|\| ''\)\.toLowerCase\(\) === 'config'/, 'config-stage proxy failures must not display an unmeasured zero duration');
assert.match(appSource, /function formatProxyCheckExtraHtml\([\s\S]*proxyStageLabel\(check\.stage\)[\s\S]*proxyErrorLabel\(check\.error\)/, 'proxy history must show localized stage and error details');
...
assert.doesNotMatch(appSource, /max_points_per_target\s*:/, 'TCP Ping history must not request a downsample budget');
...
assert.match(tcpPingChartSource, /spanGaps:\s*state\.continuousLine/, 'TCP Ping chart lines must follow the endpoint continuity toggle');
assert.match(appSource, /function lineDataset[\s\S]*spanGaps:\s*state\.continuousLine/, 'Latency source lines must follow the endpoint continuity toggle');
...
assert.match(appSource, /metricsRequestSeq/, 'metric chart requests must be guarded by a request sequence');
assert.match(appSource, /pingsRequestSeq/, 'TCP Ping chart requests must be guarded by a request sequence');
assert.match(appSource, /function clearMetricChart/, 'metric charts must clear latency-only chart state before rendering');
assert.doesNotMatch(appSource, /packetLoss:\s*true/, 'Latency chart must not render the packet-loss series');
...
assert.match(appSource, /function buildLatencyChartDatasets[\s\S]*sourceId:\s*source\.id/, 'Latency must use independently selectable source lines');
...
assert.match(styleSource, /@media \(max-width: 760px\) \{[\s\S]*\.nq-ansi\s*\{\s*font-size:\s*10px/, 'phone-sized NQ terminals must remain readable while the ANSI panel owns overflow');
...
assert.match(adminSource, /const actionTasks = \["nodequality", "ip_unlock", "backroute"\]/, 'admin must include backroute tasks in the per-target task state');
assert.match(adminSource, /data-a="task-backroute"/, 'admin must expose the fixed backroute action');
assert.match(adminSource, /data-a="task-backroute" data-task-action="backroute"/, 'backroute button must carry an explicit action independent of its label');
assert.match(adminSource, /const taskLabels = \{[\s\S]*backroute: "回程检测"/, 'backroute confirmation must use its own task label');
assert.match(adminSource, /action === "backroute"[\s\S]*固定的三网回程检测/, 'backroute confirmation must explain the actual three-carrier check');
assert.match(adminSource, /const probeUptime = typeof status\.agent_online === "boolean"/, 'admin must show Agent freshness separately from public probe status');
...
assert.match(appSource, /nextMin < fullMin/, 'chart zoom-out must keep explicit x bounds');
...
async function javascriptFiles(directory) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && entry.name.endsWith('.js') && !/\s\d+(?:\.\d+)*\.js$/i.test(entry.name)) files.push(absolute);
  }
  return files;
}
