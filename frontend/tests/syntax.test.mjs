...
assert.match(adminSource, /import \{ nqOptionsHtml, readNqOptions \} from "\.\/admin\/nq-options\.js\?v=20260804-v1119"/, 'admin must import the configurable NodeQuality option helpers');
...
assert.match(adminHtml, /href="\/admin\.css\?v=20260804-v1119"/, 'custom admin paths must load CSS from the site root');
...
assert.match(adminHtml, /src="\/js\/admin-bootstrap\.js\?v=20260804-v1119"/, 'admin login must install a startup failure guard');
assert.match(adminHtml, /src="\/js\/admin\.js\?v=20260804-v1119"/, 'custom admin paths must load the admin module from the site root');
...
assert.match(adminSource, /install-command\.js\?v=20260804-v1119/, 'Agent and Latency install clipboard fixes must use the current cache key');
...
assert.match(indexHtml, /app\.js\?v=20260804-v1119/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260804-v1119/, 'frontend CSS cache key must publish the current release');
...
assert.match(tcpPingChartSource, /spanGaps:\s*false/, 'TCP Ping chart lines must break at packet-loss gaps');
assert.match(appSource, /function lineDataset[\s\S]*spanGaps:\s*false/, 'Latency source lines must break at packet-loss gaps');
