...
assert.match(adminHtml, /href="\/admin\.css\?v=20260909-usage1"/, 'custom admin paths must load CSS from the site root');
...
assert.match(adminHtml, /src="\/js\/admin\.js\?v=20260910-repair1"/, 'custom admin paths must load the current admin module from the site root');
...
assert.match(adminSource, /install-command\.js\?v=20260908-backroute1/, 'Agent and Latency install clipboard fixes must use the current cache key');
...
assert.match(indexHtml, /app\.js\?v=20260910-backroute4/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260821-themecfg2/, 'frontend CSS cache key must publish the current release');
assert.match(indexHtml, /data-metric="proc">进程<\/button>/, 'metric tabs must expose the process chart');
