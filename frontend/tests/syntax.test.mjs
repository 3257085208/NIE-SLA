...
assert.match(adminSource, /import \{ nqOptionsHtml, readNqOptions \} from "\.\/admin\/nq-options\.js\?v=20260804-v11111"/, 'admin must import the configurable NodeQuality option helpers');
...
assert.match(adminHtml, /href="\/admin\.css\?v=20260804-v11111"/, 'custom admin paths must load CSS from the site root');
...
assert.match(adminHtml, /src="\/js\/admin-bootstrap\.js\?v=20260804-v11111"/, 'admin login must install a startup failure guard');
assert.match(adminHtml, /src="\/js\/admin\.js\?v=20260804-v11111"/, 'custom admin paths must load the admin module from the site root');
...
assert.match(adminSource, /install-command\.js\?v=20260804-v11111/, 'Agent and Latency install clipboard fixes must use the current cache key');
...
assert.match(indexHtml, /app\.js\?v=20260804-v11111/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260804-v11111/, 'frontend CSS cache key must publish the current release');
...
assert.match(styleSource, /@media \(max-width: 560px\) \{[\s\S]*\.nq-ansi-panel,[\s\S]*\.nq-network-scroll\s*\{\s*overflow-x:\s*auto[\s\S]*\.nq-ansi-panel \.nq-ansi\s*\{[\s\S]*white-space:\s*pre-wrap[\s\S]*word-break:\s*break-word/, 'mobile ANSI panels must wrap adaptively instead of forcing horizontal scrolling');
assert.match(styleSource, /@media \(max-width: 560px\) \{[\s\S]*\.nq-report-pre\s*\{[\s\S]*white-space:\s*pre[\s\S]*word-break:\s*normal/, 'mobile text reports must preserve terminal columns inside local horizontal scrollers');
...
assert.match(adminCss, /@media \(max-width: 560px\) \{[\s\S]*\.nq-ansi-panel,[\s\S]*\.nq-network-scroll\s*\{\s*overflow-x:\s*auto[\s\S]*\.nq-ansi-panel \.nq-ansi\s*\{[\s\S]*white-space:\s*pre-wrap[\s\S]*word-break:\s*break-word/, 'admin mobile ANSI panels must wrap adaptively instead of forcing horizontal scrolling');
assert.match(adminCss, /@media \(max-width: 560px\) \{[\s\S]*\.nq-report-pre\s*\{[\s\S]*white-space:\s*pre[\s\S]*word-break:\s*normal/, 'admin mobile text reports must preserve terminal columns inside local horizontal scrollers');
