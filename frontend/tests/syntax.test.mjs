...
assert.match(adminSource, /import \{ nqOptionsHtml, readNqOptions \} from "\.\/admin\/nq-options\.js\?v=20260918-proxy19"/, 'admin must import the configurable NodeQuality option helpers');
...
assert.match(adminHtml, /href="\/admin\.css\?v=20260918-proxy19"/, 'custom admin paths must load CSS from the site root');
...
assert.match(adminHtml, /src="\/js\/admin-bootstrap\.js\?v=20260918-proxy19"/, 'admin login must install a startup failure guard');
assert.match(adminHtml, /src="\/js\/admin\.js\?v=20260918-proxy19"/, 'custom admin paths must load the current admin module from the site root');
...
assert.match(adminSource, /install-command\.js\?v=20260918-proxy19/, 'Agent and Latency install clipboard fixes must use the current cache key');
...
assert.match(indexHtml, /app\.js\?v=20260918-proxy19/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260918-proxy19/, 'frontend CSS cache key must publish the current release');
...
assert.match(appSource, /import \{ targetSlaPercentage \} from '\.\/js\/shared\/sla\.js\?v=20260918-proxy19'/, 'VPS rows must calculate SLA with the tested shared helper');
