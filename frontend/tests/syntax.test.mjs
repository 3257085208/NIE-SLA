...
assert.match(adminHtml, /admin\.css\?v=20260730-v1040/, 'admin CSS cache key must publish the current release');
assert.match(adminHtml, /js\/admin\.js\?v=20260730-v1040/, 'admin JS cache key must publish the credential-safe install flow');
assert.match(adminHtml, /id="pingIntervalSec"[^>]*type="number"[^>]*min="1"[^>]*max="300"[^>]*step="1"/, 'Ping settings must expose an integer interval from 1 to 300 seconds');
...
assert.match(indexHtml, /app\.js\?v=20260730-v1040/, 'frontend cache key must publish the current release');
assert.match(indexHtml, /style\.css\?v=20260730-v1040/, 'frontend CSS cache key must publish the current release');
...
assert.match(appSource, /include_loss:\s*['"]1['"][\s\S]*pingLossOverlayPlugin\(\)/, 'Ping history must request and render the lossless loss-event overlay');
assert.match(appSource, /function pingLossOverlayPlugin[\s\S]*step === 1 && count >= 3[\s\S]*fillRect/, 'continuous one-second outages must render as compact exact spans');
