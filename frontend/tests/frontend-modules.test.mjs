...
import { NQ_OPTION_DEFAULTS, nqOptionsHtml, normalizeNqOptions, readNqOptions } from '../js/admin/nq-options.js';
...
assert.deepEqual(normalizeNqOptions({ hardware: 'v', ip: 'n', net: 'l', route: 'n', accelerator: 'eo' }), { hardware: 'v', ip: 'n', net: 'l', route: 'n', accelerator: 'auto' });
assert.deepEqual(normalizeNqOptions({ hardware: 'x', ip: 'maybe', net: 'z', route: '', accelerator: 'evil' }), NQ_OPTION_DEFAULTS);
const nqOptionsMarkup = nqOptionsHtml({ hardware: 'v', ip: 'n', net: 'l', route: 'n', accelerator: 'eo' });
const nqOptionsMarkupCf = nqOptionsHtml({ hardware: 'v', ip: 'n', net: 'l', route: 'n', accelerator: 'cf' });
assert.match(nqOptionsMarkup, /data-nq-option="hardware"/);
assert.match(nqOptionsMarkup, /value="v" selected/);
assert.match(nqOptionsMarkup, /运行 HardwareQuality 测试/);
assert.match(nqOptionsMarkup, /data-nq-option="accelerator"/);
assert.match(nqOptionsMarkupCf, /value="cf" selected/);
assert.doesNotMatch(nqOptionsMarkup, /value="eo"/);
assert.match(nqOptionsMarkup, /加速源/);
const fakeNqRoot = { querySelector(selector) { const key = selector.match(/data-nq-option="([^"]+)"/)?.[1]; return { value: { hardware: 'v', ip: 'n', net: 'l', route: 'n', accelerator: 'cf' }[key] }; } };
assert.deepEqual(readNqOptions(fakeNqRoot), { hardware: 'v', ip: 'n', net: 'l', route: 'n', accelerator: 'cf' });
assert.deepEqual(readNqOptions({}), NQ_OPTION_DEFAULTS);
...
assert.match(renderNqAnsiHtml('操作系统/内核：        Debian GNU/Linux 12 (bookworm)', 24), /\n {13,}/, 'continuation lines must align to the value column after a label colon');
assert.match(renderNqAnsiHtml('  自由文本行内容超过预算就折行', 10), /\n {2}/, 'plain indented lines must keep their leading indent when wrapped');
