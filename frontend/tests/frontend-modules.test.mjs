...
assert.deepEqual(normalizeNqOptions({ hardware: 'v', ip: 'n', net: 'l', route: 'n' }), { hardware: 'v', ip: 'n', net: 'l', route: 'n' });
assert.deepEqual(normalizeNqOptions({ hardware: 'x', ip: 'maybe', net: 'z', route: '' }), NQ_OPTION_DEFAULTS);
const nqOptionsMarkup = nqOptionsHtml({ hardware: 'v', ip: 'n', net: 'l', route: 'n' });
...
const fakeNqRoot = { querySelector(selector) { const key = selector.match(/data-nq-option="([^"]+)"/)?.[1]; return { value: { hardware: 'v', ip: 'n', net: 'l', route: 'n' }[key] }; } };
assert.deepEqual(readNqOptions(fakeNqRoot), { hardware: 'v', ip: 'n', net: 'l', route: 'n' });
