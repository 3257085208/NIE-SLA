...
assert.match(workflow, /--untracked-files=all/, 'manual bootstrap adoption must require a repository with no local changes');
assert.ok(workflow.includes("--exclude='.dev.vars'"), 'online updates must preserve local secret files');
