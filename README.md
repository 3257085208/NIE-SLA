...
NIE-SLA 的优势不是某一个孤立功能，而是把“公开状态页、Cloudflare 外部探测、VPS 系统遥测、多来源网络测量和可扩展前端”放进同一套可自托管系统：

- **Cloudflare 原生控制面**：Worker、D1、R2、Pages 与 Durable Objects 承担 API、调度、历史和前端，不需要再购买一台中心 VPS，也不需要为管理面开放服务器端口。
- **纯 Rust VPS Agent**：生产 Agent 是单文件 Rust 程序，不依赖 Python、Node.js 或 Docker；VPS 只主动发起 HTTPS 请求，不监听入站管理端口。这里的“纯 Rust”特指 VPS Agent，Worker 与前端仍使用 JavaScript。
- **不止判断在线**：Agent 本地保持 1 秒采样，覆盖 CPU、内存、磁盘、负载、网络、磁盘 IO、连接数、进程、线程与流量；网络异常时由本地持久队列重试，而不是直接丢掉整段历史。
- **硬件温度可视化**：在宿主机暴露传感器的情况下，可采集并展示 CPU、GPU、主板、硬盘和芯片组温度；虚拟机中不可靠的伪温度会被前端过滤。
- **四条监控链路**：Cloudflare HTTP/TCP、VPS Agent 心跳与指标、VPS 侧 TCP Ping、外部 Latency Agent 各自回答不同问题，避免把“CF 到目标不可达”误判成“VPS 已关机”。
- **面向 100 台级规模**：没有写死 50 或 100 台的产品授权限制。按当前免费额度模型，5 分钟上传的保守理论上限约为 110 台，建议长期稳定规划在 80 台以内；详细假设和每项额度见后文，扩容不是一句口号。
- **主题、插件与开发者 API**：后台分别上传主题包和插件包；CSS 主题可重塑视觉，隔离画布主题可接管完整布局，插件通过受控公开 API 扩展能力。ZIP 路径、Manifest、权限、CSP、SHA-256 与回滚规则均有开发规范。
- **安装与更新链路可验证**：支持 Deploy to Cloudflare、部署脚本和后台生成的单节点安装命令；Agent 自动更新由后台控制，并同时验证 manifest 固定哈希、二进制 SHA-256 与程序自报版本。
- **公开面与管理面分离**：每台 Agent 使用与 Target 绑定的 scoped Token，后台支持 TOTP；公共 API 会裁剪敏感字段，扩展运行在 CSP、sandbox、Permissions Policy 和内容类型约束下。

### 与两类常见方案的能力侧重对比

下表比较的是产品形态，而不是断言每个项目都完全相同。Cloudflare 同类列来自 2026-07 对公开项目 README、部署文档和功能列表的实际核对，不列出具体项目名称；“传统中心式探针”代表需要自行运行 Server/Dashboard 的常见方案。

| 维度 | NIE-SLA | 常见 Cloudflare 状态页/探测器 | 传统中心式 VPS 探针 |
| --- | --- | --- | --- |
| 控制面 | Cloudflare Worker + D1 + R2 + Pages + DO | 通常是 Worker + D1/KV + Pages | 自建 Server、数据库与 Web 端口 |
| 外部可用性 | Cloudflare HTTP/TCP，可选区域探测 | 通常擅长 HTTP/TCP、状态页和告警 | 常由中心服务器或 Agent 发起，取决于部署位置 |
| VPS 系统遥测 | Rust Agent，1 秒本地采样、批量上传、持久重试 | 多数以 URL/端口可用性为主，不附带完整主机 Agent | 核心强项，通常实时性和主机管理能力更丰富 |
| 温度与硬件信息 | CPU/GPU/主板/硬盘/芯片组，按传感器能力显示 | 通常不采集宿主机硬件 | 依 Agent 与平台而定 |
| 网络视角 | CF 探测 + 每台 VPS TCP Ping + 外部 Latency Agent | 通常以 CF 区域视角为主 | 通常有 Agent Ping，但不天然包含 CF 边缘视角 |
| 高频历史 | D1 保存业务状态，R2 保存原始 Metrics/Ping 与快照 | 常以 D1/KV 保存检查结果 | 保存在自建数据库，容量由中心服务器决定 |
| 前端扩展 | 后台上传主题/插件 ZIP，提供规范化 Manifest 与公开 API | 多为配置项、源码定制或内置主题，能力因项目而异 | 主题/插件生态因项目而异，成熟项目通常更丰富 |
| 节点凭据 | 每节点 scoped Token，安装与更新包双重哈希校验 | 通常使用部署 Secret 或管理会话，粒度因项目而异 | 通常是 Agent Token/Secret，远程运维会扩大权限边界 |
| 远程运维 | 刻意不提供 Web Shell 或任意命令执行，聚焦监控与展示 | 通常聚焦可用性，不提供主机控制 | Web 终端、任务和命令执行往往更强 |
| 容量依据 | README 给出 50/100/200 台的请求、D1、R2 数学与边界条件 | 常以监控项上限或免费额度说明为主 | 取决于中心服务器 CPU、内存、数据库和带宽 |

### 怎么选

- 需要 **免维护中心服务器、公开状态页、Cloudflare 外部视角和完整 VPS 遥测**，同时不希望开放远程控制面时，NIE-SLA 更契合。
- 只需要网站/API 可用性、证书或域名到期、很多通知渠道时，专门的 Cloudflare uptime 工具通常更简单，部分能力也可能更成熟。
- 需要 Web 终端、批量命令、计划任务、复杂权限或秒级在线控制时，传统中心式探针更合适；NIE-SLA 有意不把远程执行放进 Agent 权限边界。
- 这三类方案可以并存。NIE-SLA 的 External Latency Agent 和公开 API 也适合作为现有监控体系的补充，而不要求一次性替换全部工具。
