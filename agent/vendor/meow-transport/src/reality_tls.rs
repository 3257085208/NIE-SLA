...
async fn reality_handshake(
    ...
    loop {
        // A TLS record may contain the complete server flight. Consume every
        // handshake message already buffered before waiting for another
        // record; otherwise a normal record containing EncryptedExtensions,
        // Certificate, CertificateVerify and Finished leaves the client
        // waiting forever after the first message.
        let msg = loop {
            if let Some(message) = pop_handshake_message(&mut handshake_buf) {
                break message;
            }
