...
见 `agent/README_zh.md` 的环境变量表。核心项：`NIE_SLA_API_BASE`、`NIE_SLA_AGENT_TOKEN`、`NIE_SLA_AGENT_ID`、`NIE_SLA_SAMPLE_SEC`、`NIE_SLA_INTERVAL_SEC`、`NIE_SLA_PING_SEC`。旧 `NSTATUS_*` 名称只作为升级兼容回退；`NIE_SLA_ALLOW_INSECURE_HTTP=1` 只用于可信私网调试。
