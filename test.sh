...
run_shell "fixed Beta tasks only" "cd '$ROOT' && grep -q 'https://run.NodeQuality.com' agent/src/tasks.rs && grep -q 'https://IP.Check.Place' agent/src/tasks.rs && grep -q 'nodequality_stdin' agent/src/tasks.rs && ! grep -q 'NODEQUALITY_TIMEOUT_SEC' agent/src/tasks.rs && grep -q 'timeout_sec: null' worker/src/admin/agent-tasks.js && grep -q 'f\\\\ny\\\\ny\\\\ny' agent/src/tasks.rs && grep -q 'IP_UNLOCK_ARGS.*\[\"-4\", \"-n\", \"-p\"\]' agent/src/tasks.rs && ! grep -R -E 'NSTATUS_UNLOCK_CHECK|install_unlock_deps' cftz agent/cftz frontend/cftz && ! grep -R -E 'body\?\.(command|script|args|stdin)|body\[(.command.|.script.|.args.|.stdin.)\]' worker/src/admin/agent-tasks.js"
