...
run_shell "Linux ICMP uses a bounded raw-socket capability" "cd '$ROOT' && grep -q 'setcap cap_net_raw=ep' agent/setup.sh && grep -q 'setcap cap_net_raw=ep' agent/update.sh && grep -q 'setcap cap_net_raw=ep' agent/cftz && grep -q 'CapabilityBoundingSet=CAP_NET_RAW' agent/setup.sh && grep -q 'AmbientCapabilities=CAP_NET_RAW' agent/setup.sh && grep -q 'CapabilityBoundingSet=CAP_NET_RAW' agent/cftz && grep -q 'AmbientCapabilities=CAP_NET_RAW' agent/cftz && ! grep -R -q 'ping_group_range' agent/setup.sh agent/update.sh agent/cftz"
