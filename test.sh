...
run_shell "fixed tasks require a dedicated host identity and namespaces" "cd '$ROOT' && grep -q 'TASK_USER.*nstatus-task' agent/src/tasks.rs && grep -q 'PR_SET_NO_NEW_PRIVS' agent/src/tasks.rs && grep -q 'setgroups' agent/src/tasks.rs && grep -q 'setresgid' agent/src/tasks.rs && grep -q 'setresuid' agent/src/tasks.rs && grep -q 'Command::new(\"unshare\")' agent/src/tasks.rs && ! grep -q 'Command::new(\"setpriv\")' agent/src/tasks.rs && grep -q '\"--map-root-user\"' agent/src/tasks.rs && grep -q '\"-k\"' agent/src/tasks.rs && grep -q 'prepare_task_sandbox_ownership' agent/src/tasks.rs && grep -q 'id -g \"\$user\"' agent/setup.sh && grep -q 'create_system_user \"\$TASK_USER\"' agent/setup.sh && ! grep -q 'spawn_ip_unlock_fallback' agent/src/main.rs"
