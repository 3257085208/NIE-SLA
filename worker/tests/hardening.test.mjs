...
assert.match(wranglerSource, /^ALLOW_WORKERS_DEV = "false"$/m, 'production must explicitly reject the workers.dev hostname as defence in depth');
assert.match(indexSource, /ALLOW_WORKERS_DEV \?\? 'true'/, 'self-hosted deployments must keep workers.dev available by default');
assert.match(indexSource, /workersDevFlag === 'false' \|\| workersDevFlag === '0'/, 'the workers.dev hostname must only be rejected when explicitly disabled');
