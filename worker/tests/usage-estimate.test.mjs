...
test('estimate stays within metered reality envelopes (2026-09-16 calibration)', () => {
  ...
  assert.equal(r.model_version, 'usage-model-embedded-v1.3.4');
  assert.equal(r.inputs.public_rps, 0.42);
  assert.ok(r.estimates.workers_calls > 28_000 && r.estimates.workers_calls < 42_000, `workers ${r.estimates.workers_calls}`);
  assert.ok(r.estimates.do_requests > 38_000 && r.estimates.do_requests < 58_000, `do requests ${r.estimates.do_requests}`);
  assert.ok(r.estimates.do_rows_written > 28_000 && r.estimates.do_rows_written < 55_000, `do rows ${r.estimates.do_rows_written}`);
  assert.ok(r.estimates.d1_rows_written > 80_000 && r.estimates.d1_rows_written < 150_000, `d1 written ${r.estimates.d1_rows_written}`);
  assert.ok(r.estimates.d1_rows_read > 2_800_000 && r.estimates.d1_rows_read < 4_500_000, `d1 read ${r.estimates.d1_rows_read}`);
  ...
  assert.ok(r.quota.do_requests.pct > 30 && r.quota.do_requests.pct < 70, 'do request quota pct visible');
