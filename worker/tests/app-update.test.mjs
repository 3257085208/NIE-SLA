...
const manifest = {
  ...
  version: '1.1.11',
  source_ref: 'app-v1.1.11',
  published_at: '2026-07-25T12:00:00Z',
  title: 'NIE-SLA 1.1.11',
  changelog: ['更新中心', '安全修复'],
...
assert.equal(parseAppUpdateManifest(manifest).source_ref, 'app-v1.1.11');
...
assert.equal(info.current_version, '1.1.11');
assert.equal(info.latest_version, '1.1.11');
...
assert.equal(fallbackInfo.current_version, '1.1.11');
assert.equal(fallbackInfo.latest_version, '1.1.11');
...
assert.equal(staleInfo.latest_version, '1.1.11');
