...
export async function ensureV6Schema(env) {
  ...
  // /api/agent/tasks every 10 minutes. Recording that contact (throttled by
  // SQL, no extra read) lets the status page distinguish "Agent telemetry is
  // stale" from "the whole VPS/Agent is offline".
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS agent_contacts (
    agent_id TEXT PRIMARY KEY,
    manager_seen_at INTEGER NOT NULL,
  ...
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS debug_logs (
    id TEXT PRIMARY KEY,
    ts INTEGER NOT NULL,
  ...
  await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_debug_logs_ts ON debug_logs(ts DESC)`).run();
  await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_debug_logs_actor_ts ON debug_logs(actor, ts DESC)`).run();
