...
export async function syncEnvTargets(env, opts = {}) {
  ...
  for (const item of list) {
    ...
    const hasTrafficResetDay = hasOwn(item, 'traffic_reset_day');
    seenIds.add(id); ids.push(id); normalizedItems.push({
      ...
      hasTrafficResetDay, trafficResetDay: hasTrafficResetDay ? normalizeTrafficResetDay(item.traffic_reset_day) : 1,
    });
  }
  for (const { id, normalized, hasTrafficEnabled, trafficEnabled, hasTrafficQuota, trafficQuotaGb, hasTrafficMode, trafficMode, hasTrafficResetDay, trafficResetDay } of normalizedItems) {
    const previous = hasTrafficResetDay
      ? await env.DB.prepare(`SELECT traffic_reset_day FROM targets WHERE id = ?`).bind(id).first()
      : null;
    await env.DB.prepare(`INSERT INTO targets (id, name, group_name, type, target_host, target_port, url, method, expected_status, timeout_ms, interval_sec, probe_region, enabled, no_public_ip, created_at, updated_at, traffic_enabled, traffic_quota_gb, traffic_mode, traffic_reset_day) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name = excluded.name, group_name = excluded.group_name, type = excluded.type, target_host = excluded.target_host, target_port = excluded.target_port, url = excluded.url, method = excluded.method, expected_status = excluded.expected_status, timeout_ms = excluded.timeout_ms, interval_sec = excluded.interval_sec, probe_region = excluded.probe_region, enabled = excluded.enabled, no_public_ip = excluded.no_public_ip, updated_at = excluded.updated_at, traffic_enabled = CASE WHEN ? THEN excluded.traffic_enabled ELSE traffic_enabled END, traffic_quota_gb = CASE WHEN ? THEN excluded.traffic_quota_gb ELSE traffic_quota_gb END, traffic_mode = CASE WHEN ? THEN excluded.traffic_mode ELSE traffic_mode END, traffic_reset_day = CASE WHEN ? THEN excluded.traffic_reset_day ELSE traffic_reset_day END`).bind(id, normalized.name, normalized.group_name, normalized.type, normalized.target_host, normalized.target_port, normalized.url, normalized.method, normalized.expected_status, normalized.timeout_ms, normalized.interval_sec, normalized.probe_region, normalized.enabled ? 1 : 0, normalized.no_public_ip ? 1 : 0, now, now, trafficEnabled, trafficQuotaGb, trafficMode, trafficResetDay, hasTrafficEnabled ? 1 : 0, hasTrafficQuota ? 1 : 0, hasTrafficMode ? 1 : 0, hasTrafficResetDay ? 1 : 0).run();
    if (previous && normalizeTrafficResetDay(previous.traffic_reset_day) !== trafficResetDay) {
      const target = await env.DB.prepare(`SELECT traffic_enabled, traffic_quota_gb, traffic_mode, traffic_reset_day, expires_at FROM targets WHERE id = ?`)
        .bind(id).first();
      await rebuildAgentTrafficPeriod(env, id, target, now);
    }
  }
