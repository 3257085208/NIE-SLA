...
const ROUTES = [
  ...
  { method: 'GET', path: '/api/proxy-checks', rl: 'public' },
  ...
  { method: 'GET', path: '/api/v1/proxy-checks', rl: 'public' },
  ...
  { method: 'GET', path: '/api/retention-config', rl: 'write' },
  { method: 'PATCH', path: '/api/retention-config', rl: 'write' },
  ...
  { method: 'POST', path: '/api/admin/migration/nezha/preview', rl: 'write' },
  { method: 'POST', path: '/api/admin/migration/nezha/import', rl: 'write' },
  { method: 'POST', path: '/api/admin/migration/komari/preview', rl: 'write' },
  { method: 'POST', path: '/api/admin/migration/komari/import', rl: 'write' },
  { method: 'POST', path: '/api/admin/migration/nodeget/preview', rl: 'write' },
  { method: 'POST', path: '/api/admin/migration/nodeget/import', rl: 'write' },
  { method: 'GET', path: '/api/admin/usage-storage', rl: 'write' },
];
...
async function dispatchStatic(env, url, request, ctx) {
  ...
  if (nqMatch && m === 'GET') {
    ...
    const report = publicNodeQualityReport(target, { maskIps: publicMaskIps(env) });
  ...
  if (path === '/api/agent/config' && m === 'GET') { await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 120, 60, { bestEffort: true, keyPrefix: 'agent-config' })) return deny(); const agentId = url.searchParams.get('agent_id') || ''; await requireAgentForId(request, env, agentId); return json(await getAgentRuntimeConfig(env), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/agent/location' && m === 'POST') { await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 120, 60, { bestEffort: true, keyPrefix: 'agent-location' })) return deny(); const agentId = url.searchParams.get('agent_id') || ''; await requireAgentForId(request, env, agentId); return json(await submitAgentLocation(request, env, agentId), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/agent/tasks' && m === 'GET') { await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 120, 60, { bestEffort: true, keyPrefix: 'agent-tasks' })) return deny(); const agentId = url.searchParams.get('agent_id') || ''; await requireAgentForId(request, env, agentId); const runnerInstanceId = url.searchParams.get('runner_instance_id') || request.headers.get('x-nie-sla-runner-instance') || request.headers.get('x-nstatus-runner-instance') || ''; return json(await claimAgentTask(env, agentId, url.searchParams.get('actions') || '', runnerInstanceId), 200, env, { 'cache-control': 'no-store' }); }
  ...
  if (agentTaskResultMatch && m === 'POST') { await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 60, 60, { bestEffort: true, keyPrefix: 'agent-task-result' })) return deny(); const agentId = url.searchParams.get('agent_id') || ''; await requireAgentForId(request, env, agentId); const taskBody = await completeAgentTask(request, env, pathParam(agentTaskResultMatch[1]), agentId); await maybeLogDebugOperation(request, env, url, path, m, 200, ctx, taskBody?.task?.status === 'failed'); return json(taskBody, 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/latency-agent/targets' && m === 'GET') { await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 120, 60, { bestEffort: true, keyPrefix: 'latency-targets' })) return deny(); const nodeId = url.searchParams.get('node_id') || ''; await requireLatencyAgentForId(request, env, nodeId); return json(await getLatencyAgentTargets(env), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/latency-agent/results' && m === 'POST') { await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 120, 60, { bestEffort: true, keyPrefix: 'latency-results' })) return deny(); const identity = await requireAnyLatencyAgent(request, env); const body = await safeJson(request); const nodeId = sanitizeAgentId(body.node_id); if (identity?.type !== 'scoped' || sanitizeAgentId(identity.node_id) !== nodeId) await requireLatencyAgentForId(request, env, body.node_id); return json(await submitLatencyAgentResults(request, env, body), 200, env, { 'cache-control': 'no-store' }); }
  ...
  if (path === '/api/totp/disable' && m === 'POST') { await withAdmin(request, env); if (!await rateLimitD1(env, `totp-disable:${debugClientIp(request)}`, 10, 300)) return deny(300); return disableTOTP(request, env); }
  ...
  if (path === '/api/security/encryption/migrate' && m === 'POST') { await withAdmin(request, env); await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 3, 60, { bestEffort: true, keyPrefix: 'encryption-migrate' })) return deny(); return json(await migrateEncryptionMaterials(env), 200, env, { 'cache-control': 'no-store' }); }
  ...
  if (path === '/api/alerts/test' && m === 'POST') { await withAdmin(request, env); await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 10, 60, { bestEffort: true, keyPrefix: 'alerts-test' })) return deny(); return json(await sendTestAlert(request, env), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/alerts/check' && m === 'POST') { await withAdmin(request, env); await ensureV6Schema(env); if (!await rateLimitByIp(request, env, 6, 60, { bestEffort: true, keyPrefix: 'alerts-check' })) return deny(); return json(await runAlertChecks(env, { force: true }), 200, env, { 'cache-control': 'no-store' }); }
  ...
  if (path === '/api/agent/report-interval' && m === 'GET') { await withAdmin(request, env); return json({ ok: true, interval_sec: await getAgentReportInterval(env) }, 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/agent/report-interval' && m === 'POST') { await withAdmin(request, env); const body = await safeJson(request); try { const value = await setAgentReportInterval(env, body?.interval_sec); return json({ ok: true, interval_sec: value }, 200, env, { 'cache-control': 'no-store' }); } catch (err) { return json({ ok: false, error: err.message }, 400, env); } }
  if (path === '/api/turnstile/config' && m === 'GET') { return json(await getTurnstileConfig(env), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/turnstile/config' && m === 'POST') { await withAdmin(request, env); const body = await safeJson(request); return json(await saveTurnstileConfig(env, body), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/turnstile/verify' && m === 'POST') { if (!await rateLimitD1(env, `turnstile-verify:${debugClientIp(request)}`, 10, 300)) return deny(300); const body = await safeJson(request); const config = await getTurnstileConfig(env); if (!config.enabled || !body?.token) return json({ ok: false, error: 'Turnstile 未启用或缺少 token' }, 400, env); const verifyResponse = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ secret: await getTurnstileSecret(env), response: body.token, remoteip: debugClientIp(request) }) }); const verifyData = await verifyResponse.json().catch(() => ({})); if (!verifyData.success) return json({ ok: false, error: '人机验证失败' }, 403, env); return json({ ok: true }, 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/finance-summary' && m === 'GET') { await withAdmin(request, env); return json(await getFinanceSummary(env), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/traffic-corrections' && m === 'GET') { await withAdmin(request, env); return json({ ok: true, corrections: await listTrafficCorrections(env) }, 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/traffic-corrections' && m === 'POST') { await withAdmin(request, env); const body = await safeJson(request); return json(await saveTrafficCorrection(env, body?.agent_id, body?.rx_gb ?? 0, body?.tx_gb ?? 0), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/fleet-versions' && m === 'GET') { await withAdmin(request, env); return json(await getFleetVersions(env), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/usage-estimate' && m === 'GET') { await withAdmin(request, env); const hours = clamp(Number(url.searchParams.get('hours') || 24), 1, 168); return json(await estimateUsageFromEnv(env, hours), 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/admin/usage-storage' && m === 'GET') { await withAdmin(request, env); await ensureV6Schema(env); if (!await rateLimitD1(env, 'usage-storage', 6, 300)) return deny(300); return json(await getStorageUsage(env), 200, env, { 'cache-control': 'no-store' }); }
  ...
  if (path === '/api/retention-config' && m === 'GET') { await withAdmin(request, env); await ensureV6Schema(env); return json({ ok: true, retention_hours: await getRetentionHours(env), min_hours: RETENTION_MIN_HOURS, max_hours: RETENTION_MAX_HOURS }, 200, env, { 'cache-control': 'no-store' }); }
  if (path === '/api/retention-config' && m === 'PATCH') { await withAdmin(request, env); await ensureV6Schema(env); return json(await updateRetentionConfig(request, env), 200, env, { 'cache-control': 'no-store' }); }
  ...
  const adminTaskCancelMatch = path.match(/^\/api\/agent-tasks\/([^/]+)\/cancel$/);
  if (adminTaskCancelMatch && m === 'POST') { await withAdmin(request, env); await ensureV6Schema(env); return json(await cancelAgentTask(env, pathParam(adminTaskCancelMatch[1])), 200, env, { 'cache-control': 'no-store' }); }
...
function archiveYesterdayLocal(env) {
  const d = new Date((Math.floor(Date.now() / 1000) + timezoneOffsetMin(env) * 60 - 86400) * 1000);
