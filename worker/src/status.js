...
async function buildStatusPayload(env, url = null) {
  ...
  const targetsPromise = env.DB.prepare(`SELECT t.id, t.name, t.group_name, t.type, t.target_host, t.target_port, t.url, t.method, t.expected_status, t.timeout_ms, t.interval_sec, t.probe_region, t.enabled, t.no_public_ip, t.sort_order, t.created_at, t.updated_at, t.last_checked_at, t.expires_at, t.price, t.currency, t.billing_cycle, t.tags, t.location, t.city, t.provider, t.line_type, t.traffic_enabled, t.traffic_quota_gb, t.traffic_mode, t.traffic_reset_day, t.nq_updated_at, CASE WHEN t.nq_report IS NULL OR t.nq_report = '' THEN 0 ELSE 1 END AS has_nq FROM targets t WHERE t.enabled = 1 ORDER BY CASE WHEN t.sort_order IS NULL THEN 1 ELSE 0 END, t.sort_order, t.group_name COLLATE NOCASE, t.name COLLATE NOCASE`).all()
