...
本文规定 NStatus 只读前台插件的工程、安全和发布标准。插件不是 Worker 插件，也不是后台管理扩展。
...
本工程标准借鉴 NodeGet 的独立模块仓库、清单、统一命令和构建产物约定。NStatus 插件采用更小的权限面：没有直接 DOM、网络和写权限，因此 NodeGet 组件或插件必须重写为消息驱动的只读面板，不能原包复用。
