...
```bash
git clone https://github.com/3257085208/NIE-SLA-Agent.git
cd NIE-SLA-Agent/worker
bash deploy.sh
```
...
记录输出的 `database_id`，写入你自己的 `worker/wrangler.toml`。
...
确认 `wrangler.toml` 的 ID、bucket 名称和当前账号一致，并从 `worker/` 目录执行命令。
