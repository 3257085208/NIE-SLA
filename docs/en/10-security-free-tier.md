...
Use a unique admin password that satisfies the enforced complexity policy, enable TOTP when appropriate, protect generated install commands, keep IP masking enabled for public pages, and avoid putting passwords, sessions, or secrets in command arguments and public repositories. A dedicated `TOTP_ENCRYPTION_KEY` remains optional for deployments that want key separation. New deployments generate a random token for each Agent automatically; D1 authenticates with token hashes and encrypts the recoverable copies used to display install commands again.
